...
if (argv['_'][0] === 'setup') {
  setup(function () {
    console.log('Please run slackcheckin (<locale type>)')
    process.exit()
  })
} else {
  main()
...
function main() {
  ...
  //load env variables
  try {
    var config = JSON.parse(fs.readFileSync(path.join(osenv.home(), '.slackcheckin')))
  } catch (e) {
    console.log('slackcheckin not setup, please run `slackcheckin setup`');
    process.exit()
  }
